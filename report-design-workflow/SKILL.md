...
- Template mock data contract: for bundled dashboard templates, offline/mock filter options and business rows are written to `src/data/dashboard.dataset.json` and read through `src/data/dashboard.loader.ts` plus the data-source registry. Do not generate new `*.ts` mock/data modules for fixture rows, arrays, or payloads; TypeScript files are for config, loader/resolver logic, widget contracts, components, and utilities only.
...
- When the selected `pageShellPath` uses a bundled template, map dataset IDs to keys inside `src/data/dashboard.dataset.json` under `filterData` or `businessData`. Do not add generated TS data files as a shortcut around the template loader.
...
- Template shell preservation gate: when copying a bundled template, preserve its native navigation, filter trigger/panel/popover, toolbar placement, theme plumbing, logo slot, and shell-owned state. Update `nav`/`page`, `filters`, labels, sources, and defaults in `dashboard.config.ts` incrementally; do not replace the template shell with a custom nav/filter implementation or overwrite the default config structure unless the user explicitly asks for template-level redesign.
- Theme-control token gate: dark template pages must keep shell, grid/card surfaces, Element Plus controls, logo variant, and component scoped styles on one token system. The template runtime must set both `data-theme="dark"` and the Element Plus `.dark` class, or provide equivalent `--el-*` overrides; `screen.grid.innerBackgroundColor` and component card/input backgrounds must also be dark-compatible. Otherwise cards, `ElSelect`, `ElInput`, `ElDatePicker`, popovers, and form surfaces will render as light islands inside a dark shell.
...
- Component size and distribution gate: do not implement components that are too narrow, too small, crowded, or unreadable. For peer component groups inside one large block, apply the internal exact `M * N` matrix algorithm only when `actualTotal > 4`. For `actualTotal <= 4`, use a small-group layout based on content and block shape instead of this factor algorithm. When the algorithm applies, calculate from `layoutTotal`: normally `layoutTotal = actualTotal`; when `actualTotal` is prime, use `layoutTotal = actualTotal + 1` before calculating factor pairs. Then choose columns `M >= N`, and the smallest `M - N` among valid pairs where `layoutTotal = M * N`. This rule applies to the peer subcomponents inside the large block, not to replacing the top-level block itself. After choosing the internal matrix, check whether the outer large block must expand vertically with `heightExpansionRows = ceil(N * 2 / 3)` instead of compressing child components. Do not pad arbitrary empty slots; the only allowed spare cell is the single prime-balancing cell created when the algorithm applies to a prime count, and it must not create fake metrics or mock data. If the matrix becomes unreadable, split the group by business meaning, tabs, pagination, drawer, or another block before styling.
...
- Peer component groups inside a large block use the internal exact `M * N` distribution only when `actualTotal > 4`; prime `actualTotal` first becomes `layoutTotal = actualTotal + 1`, `layoutTotal = M * N`, `M >= N`, and `M - N` is minimal among valid factor pairs; the outer block uses `ceil(N * 2 / 3)` as the height expansion baseline when its current height cannot carry the internal rows, and components are not narrow, tiny, crowded, or unreadable.
...
- Do not generate `src/widgets/*Data.ts`, `src/data/*.ts`, or similar TS fixture modules to store mock rows in bundled template prototypes; use `src/data/dashboard.dataset.json` plus the existing loader/data-source registry.
- Do not overwrite a copied template's native navigation/filter design by generating new standalone nav bars, filter bars, or custom filter drawers. Use the selected template's `nav`/`page`, `filters`, toolbar, and local filter configuration points.
- Do not leave Element Plus controls on the default light theme inside a dark template page.
