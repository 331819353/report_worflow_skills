...
1. Z-shaped component audit.
   ...
   - Check peer component distribution: repeated cards/charts/tiles inside one parent block or sub-block group use the internal exact `M * N` layout only when `actualTotal > 4`; for `actualTotal <= 4`, they use a small-group layout. When the algorithm applies, prime `actualTotal` first becomes `layoutTotal = actualTotal + 1`, `layoutTotal = M * N`, `M >= N`, and `M - N` is minimal among valid factor pairs; then check whether the parent block must expand vertically with `heightExpansionRows = ceil(N * 2 / 3)`. If that shape is unreadable, the group is split rather than padded with arbitrary empty slots.
   ...
   - Check internal fit for composed parent blocks: summary zones, nested KPI grids, chart/table sub-blocks, comparison tiles, and small metric cells must fit inside their sub-block containers after preserving `5px` parent inset and `5px` sibling gap. Long metric titles should wrap to one or two reserved lines; do not accept critical labels/values hidden by `nowrap`, ellipsis, or `overflow:hidden` without disclosure.
...
6. Visual and runtime verification.
   ...
   - For Combo visuals, capture the whole component crop and verify shared ordered x-axis, bar metric/unit, line or target metric/unit, left/right y-axis mapping, series limit, dual-axis rationale when used, local-filter/legend separation, category-density fallback, `plotH >= CH * 0.48`, and exact tooltip/detail evidence.
   - For Detail Tables, capture the whole table crop and verify row grain, primary key/object field, default sort, prioritized `5-8` visible columns by default, readable row height, numeric right alignment, status/action column behavior, table-body row budget, fixed header/frozen columns when needed, pagination/search/export scope, local-filter limits, tooltip or row-detail disclosure, and loading/empty/error/no-permission states.
   - For complex/grouped table headers, capture the whole table crop and verify `columnTree` or nested grouped columns, business group names, real leaf fields, computed span/depth rules, unit/definition tooltip, fixed whole-header behavior, frozen row/primary columns when horizontally scrolling, component-local filter vs per-column header-filter separation, group-to-leaf alignment, density fallback, and useful body-row budget.
   - For Pivot Tables, capture the whole table crop and verify visualType `pivot`, S2/project analytical renderer, row dimensions, column dimensions, measure headers, aggregation formulas, subtotal/grand-total placement, frozen row/column headers, `pivotAreaH >= CH * 0.55`, visible-row budget, density fallback, exact cell tooltip/drilldown, and states.
   - For Gauge visuals, capture the whole component crop and verify one bounded metric, min/max/unit/current value, center value hierarchy, arc geometry, target/threshold/status semantics, local-filter separation, clamp/overflow behavior, and exact tooltip/detail evidence.
   - For funnel, flow, Sankey, graph, tree, decomposition, lineage, DuPont, and process-chain visuals, capture the stage/layer/lane title band and the first row/body of marks in the same screenshot. Funnel stage labels, value/share labels, loss/target markers, `L1`/`L2`/`L3` titles, group captions, and lane labels must reserve their own bands and stay at least 16px away from bars, node cards, node borders, child titles, badges, connectors, chart marks, legends, and edge labels.
