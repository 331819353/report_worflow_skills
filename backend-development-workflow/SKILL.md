...
description: "运行数据服务/后端开发阶段。用于中文请求包含：数据服务、后端开发、服务端、接口文档、API文档、接口实现、接口开发、后端接口、数据接口、接口联调准备、接口契约、启动后端、后端服务、本地后端、Flask服务、鉴权中间件、SSO后端接入、JWT、Access-Token校验。五阶段报表流程中，默认输入为API清单和数据模型文件，默认输出为API文档；只有用户明确要求实现/启动后端代码时才进入实现模式。不要用于从需求直接推导API清单或数据模型，那属于技术方案阶段。"
---
