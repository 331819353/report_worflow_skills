...
4. Design the test matrix with `$integration-test-case-design`, including explicit cases for control semantics, filter-linked data variation, non-default perspective schema/wording changes, cross-perspective data-chain consistency, metric display text, component internal fit, fixed-height card/navigation height budget plus DOM overflow, and matching UI baseline acceptance when those surfaces exist.
