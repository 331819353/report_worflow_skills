...
description: "运行测试联调/集成验证阶段，用于联调前后设计用例、执行验证、产出缺陷和回归标准。用户提到测试、联调测试、集成测试、验收测试、测试用例、测试结果、冒烟测试、URL可用性、接口连通性、SSO/鉴权/权限测试、前后端数据一致性、页面数据核对、筛选联动、交互/导出测试、问题复现、证据截图、阻塞项时触发。"
---
