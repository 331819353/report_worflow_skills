...
- DOM overflow evidence for domain navigation, Tabs, Segments, and first-level perspective controls at `1920x1080` and `1280x768`: each visible item/card content viewport must satisfy `scrollHeight <= clientHeight + 2` and `scrollWidth <= clientWidth + 2`. Screenshots are not a substitute for this evidence.
...
- Template/component control ownership evidence is required when a template shell exists: refresh, download/export, copy/share, global filters, period/date controls, topbar actions, local filters, and component micro actions must be mapped to one owner. Duplicate visible controls without an explicit ownership decision and disabled counterpart are `VIS-DUPLICATE-CONTROL` or `RPT-SHELL-DUPLICATE`.
- ECharts resize lifecycle evidence is required when an implemented ECharts chart is present and the owning skill asks for runtime proof. Capture source/config or DOM evidence for the approved chart renderer, `setOption`/update path, resize trigger (`ResizeObserver` preferred, or documented equivalent), and cleanup path. In the browser, measure at the design viewport and at least one different viewport/container state:

```text
chartBodyRect.width > 0
chartBodyRect.height > 0
svgOrCanvasRect.width ~= chartBodyRect.width within 2px
svgOrCanvasRect.height ~= chartBodyRect.height within 2px
axis chart plot height >= required floor when axes/legend are visible
...
If the page shell is fixed design-width and uses browser scrolling, label the result `container-resize-safe` only when the chart itself resizes correctly. Do not claim `viewport-responsive` unless breakpoints, reflow, or container-query behavior are also proven.
...
- Chart squeeze failure: a full line/bar/combo axis chart renders as a thin horizontal band, chart body is below the required floor, `grid.top + grid.bottom` consumes most of the chart height, y-axis labels stack, or gridlines merge into a stripe. Record `VIS-CHART-SQUEEZED` or `VIS-AXIS-LABEL-STACKED`.
- Chart/table crowding failure: a chart and table/list/detail preview share one card but the chart plot floor and at least `3` visible preview rows cannot both fit. Record `VIS-CHART-TABLE-CROWDING` and route the preview to Top3, drawer, tab, detail route, split block, or larger span.
...
- ECharts resize failure: chart body is zero-size at mount, the generated SVG/canvas no longer matches the chart body after resize, marks become blank/stale, legend/axis/title/state content clips after a size change, or lifecycle proof lacks resize/cleanup evidence.
- False responsiveness claim: the implementation only supports a fixed design-width shell with scroll or a one-time chart render, but the result is described as viewport-responsive without breakpoint/reflow/container-resize proof.
- Alignment intent mismatch: KPI primary values are off-center when the centered-value contract applies, or table/list/text content is forcibly centered in a way that harms scanning without an explicit centered-content spec.
