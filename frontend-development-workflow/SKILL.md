...
description: "运行前端开发/前端联调阶段。用于中文请求包含：前端开发、前端联调、前端接入接口、替换mock、去掉mock、接真实接口、接口对接、报表页面联调、仪表盘联调、数据大屏联调、前端数据接入、API接入、请求封装、响应适配、筛选参数联动、组件联动、环境变量、代理配置、CORS、登录态、SSO前端接入、前端鉴权、页面运行、启动前端、修复编译错误、修复请求错误、修复页面展示。标准输入为原型源码/前端源码加API文档、API清单、数据源说明、环境/鉴权说明或运行样例；输出为已接入真实数据的前端源码、前端功能说明和可运行URL或明确阻塞项。不要用于从零设计报表原型或后端接口实现。"
---
