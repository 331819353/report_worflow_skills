...
| 核心问题 | Executive summary | Question statement, conclusion card, decision prompt; runnable `visualType: 'text-summary'` when text-led |
| 时间 / 组织 / 范围 | Filter block / header meta | Date picker, org selector, active filter chips |
| 核心结论 | Executive summary | Text summary card, key finding list, conclusion strip; runnable `visualType: 'text-summary'` |
