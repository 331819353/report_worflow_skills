...
14. Configure modal drilldowns in `modals`. Modals use the same `layoutRows + widgets` model as the page. If filters change after a modal opens, the shell marks `context.isStale`; widgets must clear old selections or show a stale-state message.
...
16. For large widgets such as DuPont analysis, relation graphs, or wide canvases, configure `viewport` in `dashboard.config.ts` instead of implementing pan/zoom inside the widget.
...
- The shared card base is owned by the template: light theme uses white cards, 8px radius, 24px padding, and light `0 2px 10px` shadow with no hard border; dark theme uses matching black-blue tokens with the same geometry.
