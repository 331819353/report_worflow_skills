...
Typical inputs are source data, real table/view metadata, data models, database rows, SQLite fixture rows, upstream payloads, event schemas, API response expectations, frontend/prototype contracts, metric formulas, and sample rows. The output is a source-to-target transformation mapping with verification samples.
...
3. Build a source-to-response mapping.
   ...
   When the target contract comes from a prototype but real table/view metadata is available, map real source fields into the prototype/API response fields instead of copying mock names as source truth. Define SQL/view/repository expressions for renamed fields, derived metrics, enum labels, date/period conversion, permission scope, and aggregate outputs. If the mapping, quality rules, permission behavior, and performance constraints are confirmed, the real SQL-backed response path is publishable directly.
...
7. Add verification samples.
   ...
   SQLite fixture samples may be small, but they must prove that request parameters change rows, totals, rankings, pagination, drilldowns, empty states, and permission-limited results in the same way the real API contract promises. A parameter accepted by the API but not represented in fixture data is a partial or blocked transformation.
...
3. SQL/source execution plan for SQLite simulation and real table/view access, including which filters, joins, aggregates, rankings, and pagination happen at the repository/source boundary.
4. Time, aggregation, derivation, reconciliation, sorting, and export rules when applicable.
5. Verification samples or test cases for non-trivial transformations.
6. Missing information or blocked decisions that prevent safe implementation.
...
- SQLite fixture data covers parameter-driven response changes with production-like semantics even when the row count is small.
- Real table/view metadata can be transformed into prototype/API response fields through SQL/repository mapping and can become the publishable path once contract, permission, quality, and performance checks pass.
