...
export const cockpitConfig: DashboardConfig = {
  ...
  // 8. 新增组件时，需要在 src/widgets/registry.ts 注册组件，并在 src/widgets/types.ts 增加 props 类型。
  nav: [
    {
      ...
      // widgets 是块内容配置。未配置的块会保持空白占位。
      widgets: {
        A: {
          // type 对应 src/widgets/registry.ts 里的注册名。
          type: 'MetricCard',
          // title 会覆盖块标题；不写则继续显示块字符，例如 A、B。
          title: '经营收入',
          // props 会原样传给组件。不同组件的 props 类型由 src/widgets/types.ts 维护。
          props: {
            value: '128.6',
            unit: '亿',
            trend: '+12.4%',
        ...
        k: {
          ...
          // viewport 开启后，组件内容支持滚轮缩放、拖拽平移、双击复位。
          viewport: {
            pannable: true,
            zoomable: true,
            minZoom: 0.5,
  ...
  // 筛选项配置。每个 group 是一个筛选分组，每个 option 是分组里的可选值。
  // 当前模板只负责展示和记录选中值；如果要联动真实数据，可在业务组件中读取 activeFilters。
  filters: [
    {
      // 筛选分组唯一标识，不要和其他分组重复。
      id: 'cycle',
