...
description: "Run report frontend/backend integration testing from already running frontend and backend URLs. Focus on SSO, frontend display data versus backend API data consistency, and complete filter behavior. Use during the testing phase for runtime URL verification, SSO validation, token/clientId header checks, API response comparison, mock replacement verification, filter option completeness, filter-to-component binding, and end-to-end report QA before delivery. Frontend/backend source files or Git URLs are optional diagnostics inputs for root-cause analysis."
---
...
Use this skill in the testing phase after frontend and backend development are both runnable. The goal is to prove that login/auth, backend API responses, frontend displayed data, and filters work as one system, not merely that the page can render.

This skill orchestrates these testing skills:
...
It also complements `$frontend-runtime-qa-validation`, `$frontend-api-contract-validation`, `$backend-api-contract-validation`, `$report-filter-data-design`, and SSO-specific frontend/backend skills.
