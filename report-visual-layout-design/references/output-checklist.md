...
- Full axis charts are not compressed into thin bands. Normal line/bar/combo chart bodies are `>=180px`; chart + table/list, dual-axis, top-tabs, metric-strip, or target-label compositions are `>=220px` with plot height `>=140px`, unless explicitly converted to sparkline mode with axes/legend hidden.
...
- Chart + table/list composite blocks preserve both chart and preview floors. If a detail table/list needs more vertical room, move it to drawer/tab/detail route or split the block rather than squeezing the chart.
