...
description: "用于报表页面视觉布局设计、评审和修复。用户提到页面布局、页面壳、顶部栏、左侧导航、页面级筛选区、工具栏、8*N网格、区块排布、首屏层级、响应式、Haier品牌位置、图表/表格容器尺寸、hover动效裁切、空态/加载/错误态位置、元素重叠、溢出、布局不好看时触发；不负责分块标题区设计、模板工程复制、业务报表类型选择或组件细节样式。"
---
...
Use this skill to design the report page shell and visual layout. It owns page organization: header, navigation, page/global filter/control surface, toolbar, brand position, `8 * N` content grid, parent block sizing, parent-block-to-sub-block composition, first-viewport hierarchy, responsive behavior, and layout defect repair.

It does not own report-type business logic, template project assets, block-internal title/function areas, component-level styling, API/model design, or runtime QA.
...
For template-based pages, use `$report-prototype-template-management` `references/template-layout-design-system.md` as the source of truth for shared template spacing, block inner/outer padding, card radius, block-owned title/function area geometry, widget viewport, and hover/focus layout behavior. If the selected template owns filter invocation, design the native page/global filter surface and binding contract; do not add a separate filter toolbar or persistent filter bar.
...
6. Place page shell identity, navigation, page/global filters, toolbar actions, logo, and status/refresh/export/fullscreen controls. For template-based pages, map page/global filters to the template's native `filters[]` trigger/panel/popover/drawer. Do not design per-block title areas here; record each block's title/function/local-filter needs and hand them to the block/template/component owner.
